...
// @coyarzunroa
// powered by
// //https://github.com/ristofer/artekis3/blob/main/static/main.js
//////////////////////////////////////////////////
//this version: 
// to do: 
// update video play
...
let VIDEO_ROOT = "video/";//https://storage.googleapis.com/gatito/"
let videos = [];
////////////////////
function preload(){
  table = loadTable('data/20210303_mmm_data - Hoja 1.csv', 'csv');

  //hadcoding 27
videos.push(createVideo(VIDEO_ROOT+"video AaronMaureira.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_AgustinEspinosa.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_AlanGarcia.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_AlbertoEspinosa.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video Alberto.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_BenjamínZapata.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_CarlaBordagaray.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_CatrinaCapurro.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_DanielMendez.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_DavidCancino.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_EmilianoVillarroel.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_FlorenciaFolch.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_JavieraFarfan.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_JuanGonzales.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_LuciaRodríguez.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video MaríaVillegas.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video MartinaFernandez.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_MillarayFreire.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video NicolasRoman___.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_NN_1.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_NohemiMendez.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_RafaelHernandez.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_RenatoPeña.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_RenatoValdenegro.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_ReneCarpio.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_RodrigoSoto.mp4"));
videos.push(createVideo(VIDEO_ROOT+"video_SebastianAcevedo.mp4"));
...
function draw(){
  background(255,0);
  if(bgIndex>=0)image(videos[bgIndex],0,0,width,height);//audioNodes[bgIndex].background
...
function AudioNode(i){
  ...
  var base = "media/"+this.name+"/";

  this.filename1       = base+this.name+".mp3";//table.getString(this.index, 1);//natural
  this.filename2       = base+this.name+"_pitch.mp3";//+table.getString(this.index, 2);//warp
  this.filename3       = base+this.name+".jpg";//+table.getString(this.index, 3);//thumb
  this.filename4       = base+this.name+".gif";//+table.getString(this.index, 4);//bg//this.filename3;//

  this.gainVals          = [];
  this.panVals           = [];
  
  for(var j=0; j<5; j++){
    this.gainVals[j] = table.getString(this.index, 2+2*j);//
    this.panVals[j]  = table.getString(this.index, 2+2*j+1);//
  }
  ...
  this.icon       = loadImage(this.filename3);//, this.loadIcon);//allIcons[this.index];//
  //this.background = loadImage(this.filename4);//, this.loadIcon);//allIcons[this.index];//
...
///////////////////////////////////////////////////////////
      ...
      fill(0,96);var rr = random(1.0);
      rect(this.buttons[4].x,this.buttons[4].y,60*rr,padH/10);//, PI);
  ...
  this.update = function(){
    ...
    //setups
    this.gain = map(this.particle.y, +padW/2+padH/8, height-padW/2, 1,0);//padH/2+padH/6+this.particle.h/2, height-padH/2-this.particle.h/2, 1, 0);
...
function  gain2y(gain){
    return map(gain,1,0,+padW/2+padH/8,height-padW/2);//-padH/8);
  }
///////////////////////////////////////////////////////
...
// this class describes the properties of a single particle.
  constructor(i,g,p){
    this.index = i;
    this.x = pan2x(p);//audioNodes[i].panVals[globalSetup]);//random(0,width);
    this.y = gain2y(g);//audioNodes[i].gainVals[globalSetup]);//random(28,height-38);
    this.w = padW/2;
  ...
  this.draw = function(){
    ...
    for(var i=0; i<this.buttons.length; i++){
      ...
      if(i==0){
        ...
        textAlign(CENTER);
        if(i==1){
          text("SYNC", this.buttons[i].x, this.buttons[i].y+4);
